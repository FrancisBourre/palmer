...
package com.bourre.ioc.parser.factory.xml 
{
	...
	public class AttributeAsPropertyProcessor implements ContextProcessor
	{
		...
		{
			var node : XMLList = _oContext..*.( hasOwnProperty( getAttributeName( ContextAttributeList.ID ) ) && @[ContextAttributeList.ID] == id );
			
			var atts : XMLList = node.attributes( );
			for each (var o : XML in atts) 
			{
				var nodeName : String = ContextNameList.PROPERTY;
